...
If you would like, you can fine-tune the model by adjusting the number of layers, number of neurons in each layers, adjusting the learning rate, etc.

The models we produced were not fine-tuned.

Also of note is that we used a safety factor of 1.5, which is common in engineering applications. Our predictions state a level of confidence and an upper and lower range. The prediction itself can be calculated by summing the upper and lower bounds and dividing the result by 2. 
